%%%%%% preprocess_Andrew_ICA_to_processed_ERP_halved.m %%%%%%
%
...
% code, and for setting up parameters.
%
...
% 6. Creates and saves artifact rejection txt files, ERP sets,
%    and EEG sets from the first and second halves of the participant
%    data
...
for i = 1:length(sub_ids)
     ...
     pop_summary_AR_eeg_detection(EEG, [artifact_rejection_output_dir '/first_half_' EEG.subject '_art_rej_after_ICA.txt'], 'History', 'gui');
     EEG = pop_rejepoch(EEG, find(EEG.reject.rejmanual), 0);

     %% Updates EEG Data set to prepare for conversion to ERP

     [ALLEEG, EEG, CURRENTSET] = pop_newset(ALLEEG, EEG, CURRENTSET, 'setname', [EEG.subject '_final_first_half'], 'gui', 'off');

     % saves EEGset
     EEG = eeg_checkset(EEG);
     EEG = pop_saveset(EEG, 'filename', ['first_half_' sub_id '_processed_EEG.set'], 'filepath', fullfile(main_dir, 'Processed_EEGsets'));
     ...
     pop_savemyerp(ERP, 'erpname', ERP.erpname, 'filename', ['first_half_' ERP.erpname '.erp'], 'filepath', fullfile(ERP_output_dir));
     log_text{end+1} = sprintf('%s\tProcessed ERPset saved as\t%s', datestr(clock), fullfile(ERP_output_dir, ['first_half_' ERP.erpname '.erp']));

      

     %% reload full dataset of participant for second half analysis
     ...
     % and puts result out to the command window and to a file 
     EEG = pop_artextval( EEG , 'Channel',  1:34, 'Flag',  1, 'LowPass',  -1, 'Threshold', [ (-1 * threshold_abs) threshold_abs], 'Twindow', [ -300.8 1197.3] );
     pop_summary_AR_eeg_detection(EEG, [artifact_rejection_output_dir '/second_half_' EEG.subject '_art_rej_after_ICA.txt'], 'History', 'gui');


     %% Updates EEG Data set to prepare for conversion to ERP

     [ALLEEG, EEG, CURRENTSET] = pop_newset(ALLEEG, EEG, CURRENTSET, 'setname', [EEG.subject '_final_second_half'], 'gui', 'off');

     % saves EEGset
     EEG = eeg_checkset(EEG);
     EEG = pop_saveset(EEG, 'filename', ['second_half_' sub_id '_processed_EEG.set'], 'filepath', fullfile(main_dir, 'Processed_EEGsets'));
     ...
     ERP.erpname = [ERP.subject '_processed_ERP'];
     pop_savemyerp(ERP, 'erpname', ERP.erpname, 'filename', ['second_half_' ERP.erpname '.erp'], 'filepath', fullfile(ERP_output_dir));
     log_text{end+1} = sprintf('%s\tProcessed ERPset saved as\t%s', datestr(clock), fullfile(ERP_output_dir, ['second_half_' ERP.erpname '.erp']));
